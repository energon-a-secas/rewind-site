...
class SimulationUI {
    ...
    container.innerHTML = `
      <div class="profile-leaderboard">
        ${profiles.map(([key, stats]) => {
          ...
          const medal = rank === 1 ? icon('medal', { cls: 'ic-gold' }) : rank === 2 ? icon('medal', { cls: 'ic-silver' }) : rank === 3 ? icon('medal', { cls: 'ic-bronze' }) : `#${rank}`;
          return `
  ...
  async runAgentReview(type) {
    ...
    container.innerHTML = `<div class="agent-thinking">${icon('bot')} Analyzing simulation data...</div>`;
  ...
  async runBothReviews() {
    ...
    const container = document.getElementById('agentReviews');
    container.innerHTML = `<div class="agent-thinking">${icon('handshake')} Running dual analysis...</div>`;
  ...
  generateTechnicalReview(results, gaps) {
    ...
    return {
      type: 'technical',
      title: `${icon('wrench')} Technical Review`,
      emoji: icon('bot'),
      ...
      summary: issues.length > 2 ? `${icon('triangle-alert')} Multiple balance issues detected` : issues.length > 0 ? `${icon('zap')} Minor adjustments recommended` : `${icon('check')} System healthy`
    };
  }
  ...
  generateSocialReview(results, gaps) {
    ...
    return {
      type: 'social',
      title: `${icon('users')} Social Review`,
      emoji: icon('handshake'),
      ...
      summary: issues.length > 2 ? `${icon('triangle-alert')} Player experience issues detected` : issues.length > 0 ? `${icon('lightbulb')} Minor UX improvements suggested` : `${icon('check')} Players should enjoy this`
    };
  }
  ...
  generateCombinedVerdict(technical, social) {
    ...
    return 'Several issues need attention. Focus on the highest-impact recommendations first, then re-test to measure improvement.';
  }
}
