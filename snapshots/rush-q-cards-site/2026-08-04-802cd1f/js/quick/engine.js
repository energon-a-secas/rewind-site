...
import { shuffle, aiStepDelay } from './utils.js';
...
function startAgendaPick() {
  // AI can't lend (that's a human-only UI action), so keep Generous Leader out
  // of the AI pool — but the human may still pick it from their own options.
  const aiIncompatible = new Set(['Generous Leader']);
  const aiPool = shuffle(AGENDAS.filter(a => !aiIncompatible.has(a.name)));
  let idx = 0;
  const assigned = new Set();
  for (const p of state.players) {
    if (p.isAI) {
      p.agenda = aiPool[idx++ % aiPool.length];
      assigned.add(p.agenda.name);
    }
  }

  // Human picks from 2 options drawn from the full agenda list (minus any the
  // AI already took), so every agenda — including Generous Leader — is reachable.
  const humanPool = shuffle(AGENDAS.filter(a => !assigned.has(a.name)));
  state.agendaOptions = [humanPool[0], humanPool[1] || humanPool[0]];
  state.phase = 'agenda';
  render(state);
...
function startQuarter(q) {
  ...
  if (currentPlayer().isAI) {
    ...
    else setTimeout(() => runCurrentAiTurns(), aiStepDelay());
  }
}
...
function resolveCrisis(card) {
  ...
  maybeShowHint('quick-first-crisis');
...
function runCurrentAiTurns() {
  // Sim mode stays synchronous; interactive play steps turn-by-turn with a
  // render between opponents so the board is watchable (speed via settings).
  if (state._simMode) {
    while (currentPlayer().isAI && !state.players.every(p => p.hasPassed)) {
      ...
      advancePastPassed();
      if (!state.players.every(p => p.hasPassed)) dealBudgetCard(currentPlayer());
    }
    render(state);
    return;
  }
...
function stepAiTurn() {
  if (!currentPlayer().isAI || state.players.every(p => p.hasPassed)) {
    render(state);
    return;
  }
  applyCatchUpBonus(currentPlayer(), state.players);
  ...
  const d = aiStepDelay();
  if (currentPlayer().isAI) setTimeout(stepAiTurn, d);
}
...
/** Record a played card for balance-simulator meta analysis. */
...
export function doLend(personId, targetPlayerIdx, projectId) {
  ...
  // Physically move the person to the target for the quarter so they leave the
  // lender's pool (can't be double-used) and the target's scoring can find them
  // (premium counts as 2). They return to the lender at quarter end.
  const lenderIdx = state.players.indexOf(player);
  const srcIdx = player.people.indexOf(person);
  if (srcIdx >= 0) player.people.splice(srcIdx, 1);
  person._lentFrom = lenderIdx;
...
function endQuarter() {
  ...
  for (const p of state.players) {
    ...
    for (const proj of completed) {
      ...
      // Capture staffing at completion — committed/pushed are deleted below, so
      // directive checks must read these snapshots, not the live maps.
      proj._committedCount = (p.committed[proj.id] || []).length;
      proj._pushCount = p.pushed[proj.id] || 0;
      p.rep += (proj.reward || 0);
  ...
  // Return lent people to their original owner.
  for (const holder of state.players) {
    for (const person of [...holder.people]) {
      if (person._lentFrom === undefined) continue;
      const owner = state.players[person._lentFrom];
      // Remove from the borrower's committed maps and people list.
      for (const ids of Object.values(holder.committed)) {
        const cidx = ids.indexOf(person.id);
        if (cidx >= 0) ids.splice(cidx, 1);
      }
      const hidx = holder.people.indexOf(person);
      if (hidx >= 0) holder.people.splice(hidx, 1);
      delete person._lentFrom;
      delete person._lentTo;
      if (owner && owner !== holder) owner.people.push(person);
    }
  }
...
function checkDirectiveCondition(dir, player) {
  ...
  if (name === 'Resource Efficiency') {
    // Completed a project with exactly the minimum committed people (no overstaffing, no pushes).
    return player.completedProjects.some(p =>
      (p._pushCount || 0) === 0 && (p._committedCount || 0) === getMembersNeeded(p));
  }
  if (name === 'Cross-Team Excellence') return (player._lendCount || 0) >= 1;
  if (name === 'Cost-Cutting Initiative') {
    // Completed a project without spending any pushed cards.
    return player.completedProjects.some(p => (p._pushCount || 0) === 0);
  }
  return false;
}
