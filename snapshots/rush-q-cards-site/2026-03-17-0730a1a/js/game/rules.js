...
export function resolveRushQ(card, s) {
  ...
  // Appropriated Glory: steal half credit from opponent's completed project
  if (name === 'Appropriated Glory') {
    // Find opponent with completed projects
    const opponents = s.players.filter(p => p !== player);
    const opponentsWithCompleted = opponents.filter(p => p.projects.some(pr => pr.isCompleted));

    if (!opponentsWithCompleted.length) {
      addLog(s, `${player.name} played "${card.name}" — no opponents with completed projects.`);
      return { outcome: 'No targets', detail: 'No opponent has completed a project' };
    }

    // AI: target player with highest-reward completed project
    if (player.isAI) {
      let bestTarget = null;
      let bestReward = 0;
      let bestProject = null;

      for (const opp of opponentsWithCompleted) {
        const completed = opp.projects.filter(pr => pr.isCompleted && pr.reward > 0);
        for (const proj of completed) {
          if (proj.reward > bestReward) {
      ...
      if (bestTarget && bestProject) {
        const stolen = Math.ceil(bestProject.reward / 2);
        bestTarget.reputation -= stolen;
        player.reputation += stolen;
        addLog(s, `Appropriated Glory → ${player.name} stole ${stolen} rep from ${bestTarget.name}'s "${bestProject.name}".`);
        return { outcome: `+${stolen} reputation`, detail: `Stole half the credit for "${bestProject.name}"` };
      }
    }

    // Human: would need dialog - for now, AI behavior
    const target = opponentsWithCompleted[0];
    const completed = target.projects.filter(pr => pr.isCompleted && pr.reward > 0);
    if (completed.length) {
      const proj = completed[0];
      const stolen = Math.ceil(proj.reward / 2);
      target.reputation -= stolen;
      player.reputation += stolen;
      addLog(s, `Appropriated Glory → ${player.name} stole ${stolen} rep from ${target.name}'s "${proj.name}".`);
      return { outcome: `+${stolen} reputation`, detail: `Stole half the credit for "${proj.name}"` };
    }
  }
...
/** Resolve a Layoff card. */
export function resolveLayoff(card, s) {
  const target = s.players[s.layoffTargetIndex];
  if (!target) return;
  ...
  s.layoffTargetIndex = (s.layoffTargetIndex + 1) % s.players.length;
}
