...
import { render, renderAssignModal, renderTradeModal, showCardDetail } from './render.js';
...
window._updateTradeTotal = function() {
  ...
  if (costText) {
    ...
    if (btn) btn.disabled = total < cost || ids.length === 0;
  }
};
...
window._onDragPerson = function(e, personId) {
  ...
  const card = e.target.closest('.person-row');
  if (card) {
...
window._showCardHover = function(e, cardId) {
  ...
  if (!preview) return;
  preview.innerHTML = renderCardHTML(card, { size: 'sm' });
  const rect = e.currentTarget.getBoundingClientRect();
  const previewH = 220;
  const top = Math.max(8, rect.top - previewH - 8);
