...
export function renderDetail(card) {
  ...
  const statItems = [];
  if (card.value)    statItems.push(['Value', card.value]);
  if (card.deadline) statItems.push(['Deadline', `Q${card.deadline}`]);
  ...
  if (card.deck)             statItems.push(['Deck', card.deck]);
    ...
    <div class="modal-content">
      ...
      <div class="card-detail">
        ...
        <div class="card-detail-body">
          <span class="type-badge" style="background:${color}22;color:${color}">${escHtml(card.type)}</span>
          ...
          <div class="stat-grid">
            ${statItems.map(([label, val]) =>
              `<div class="stat-item"><span class="stat-label">${label}</span><span class="stat-value">${escHtml(String(val))}</span></div>`
            ).join('')}
          </div>
        </div>
      </div>
