...
function collectGameResult() {
  ...
  return {
    ...
    // New Tier 7 metrics
    turnCount: state.currentQuarter * state.players.length,
  };
}
...
function analyzeResults(results, numPlayers) {
  ...
  for (const r of results) {
    ...
    for (let i = 0; i < numPlayers; i++) {
      ...
      // Track dead turns and options per turn
      if (r.players[i].turnOptions) {
  ...
  return {
    ...
    decisionComplexity: totalTurns > 0 ? (totalOptionsPerTurn / totalTurns).toFixed(2) : '0',
    deadTurnRate: totalTurns > 0 ? ((totalDeadTurns / totalTurns) * 100).toFixed(1) + '%' : '0%',
...
function analyzeCardBalance(cardPlayStats, winnerDeckMeta, totalGames) {
  ...
  analysis.metaHealth = ((viableCards / analysis.allCards.length) * 100).toFixed(1) + '%';
