...
/** Tier 3A: Project synergy definitions - bonus rep for completing related projects */
const PROJECT_SYNERGIES = [
  {
    combo: ['API Redesign', 'Mobile App'],
    bonus: 2,
    message: 'API enables mobile launch'
  },
  {
    combo: ['Database Migration', 'API Redesign'],
    bonus: 2,
    message: 'Modern stack unlocks performance'
  },
  {
    combo: ['Hiring Spree', 'Team Building'],
    bonus: 1,
    message: 'New hires integrate smoothly'
  },
  {
    combo: ['Security Audit', 'Compliance Check'],
    bonus: 2,
    message: 'Security culture pays dividends'
  },
  {
    combo: ['Performance Optimization', 'Infrastructure Upgrade'],
    bonus: 2,
    message: 'Hardware + software = maximum speed'
  },
  {
    combo: ['User Research', 'UX Redesign'],
    bonus: 1,
    message: 'Data-driven design succeeds'
  },
  {
    combo: ['Code Review Process', 'Documentation Sprint'],
    bonus: 1,
    message: 'Quality culture compounds'
  },
  {
    combo: ['Technical Debt Sprint', 'Refactoring'],
    bonus: 1,
    message: 'Clean code enables velocity'
  }
];
...
export const startGame = withErrorBoundary(async function startGame_inner(playerName, aiCount, totalQuarters = 8) {
  ...
  // Assign management style traits
  assignTraits(state);
  ...
  for (let pi = 0; pi < state.players.length; pi++) {
    ...
    for (const proj of p.projects) {
      ...
      if (effectiveAssigned.length === 0) {
        ...
        // Fully staffed - progress equal to required members (expected pace)
        // Apply tech debt penalty first
        const techDebt = proj.techDebt || 0;
        const progressAfterDebt = Math.max(0, membersNeeded - techDebt);
