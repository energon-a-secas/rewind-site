...
import { drawCard, refillMarket, getUnassignedPeople, getAssignedPeople, playCard, spendRetroToken } from './engine.js';
...
export async function runAiTurn(player, s) {
  ...
  skillCards.sort((a, b) => {
    const aScore = aiSkillCardScore(a);
    const bScore = aiSkillCardScore(b);
    return bScore - aScore;
  });
  // Play up to max skills per turn (trait-dependent, default 2)
  const maxSkills = traitMaxSkills(player);
  let skillsPlayed = 0;
