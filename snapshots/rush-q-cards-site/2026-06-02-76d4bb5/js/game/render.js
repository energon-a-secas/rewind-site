...
/** Convert card name to image src path (relative to /game/). */
...
function renderBoard(s) {
  ...
  if (s.currentQuarter === 1 && !s._tutorialDismissed) {
    html += `<div class="tutorial-hint" id="tutorial-hint">
      <strong>Welcome!</strong> Play cards from your hand below. Assign people to projects, then end your turn.
      <button class="tutorial-dismiss" onclick="event.stopPropagation();this.parentElement.remove();window._dismissTutorial()">Got it</button>
    </div>`;
  }
...
function renderActions(s) {
  ...
  if (s.markets.projectQueue.length || (s.forHireUnlocked && s.markets.forHire.length)) {
    html += `<button class="btn btn-trade" onclick="window._showTrade()" aria-label="Trade cards (keyboard: T)"><img src="/assets/icons/ui/trade.svg" alt="" aria-hidden="true" style="width: 16px; height: 16px; margin-right: 6px; vertical-align: -2px;">Trade <kbd>T</kbd></button>`;
  }
  ...
  html += `<button class="btn btn-secondary btn-sm" onclick="window._showTutorial()" title="Show Tutorial" aria-label="Show tutorial (keyboard: H)"><img src="/assets/icons/ui/help.svg" alt="" aria-hidden="true" style="width: 14px; height: 14px; margin-right: 4px; vertical-align: -2px;">Tutorial</button>`;
