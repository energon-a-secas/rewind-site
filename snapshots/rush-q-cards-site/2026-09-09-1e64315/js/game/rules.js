...
import { addLog, recordIncomingHit } from './state.js';
...
/** Talent negation riders: event name → the Talent whose printed text negates
 *  it while assigned to a project. */
const TALENT_EVENT_RIDERS = {
  'Skills Gap': 'Tech Lead',
  'Failed Release': 'QA',
  'Changing Requirements': 'Senior Lv. 3',
  'Onboarding': 'Senior Lv. 1.5',
  'Attrition': 'Contractor',
  'RFP': 'Manager (Starter)',
};
// Punished Senior's rider names Scope Creep, which is a Rush Q card resolved
// globally in resolveRushQ rather than a per-player Event — so it cannot go in
// the map above. It is honoured as a per-player exemption there instead.
const SCOPE_CREEP_RIDER = 'Punished Senior';

/** True when the player has a person with this name assigned to any project. */
function hasAssignedTalent(player, talentName) {
  for (const ids of Object.values(player.assignments || {})) {
    for (const id of ids) {
      const person = player.people.find(x => x.id === id);
      if (person && person.name === talentName) return true;
    }
  }
  return false;
}
...
function isChoiceCard(card) {
  ...
  // 'you may choose' covers Company Priorities / Meetings ("You may choose to
  // do both.") whose printed either/or was previously auto-resolved.
  return et.includes('choice') || skill.includes('choose one') || skill.includes('choose:') || skill.includes('you may choose');
}
...
export async function resolveEvent(card, player, s) {
  ...
  // Played event shield: consume one charge to negate the event outright.
  // Checked after Foresight so free immunity is always used before spending a
  // shield.
  if ((player.shieldActive || 0) > 0) {
    player.shieldActive--;
    addLog(s, `${player.name}'s event shield negates "${name}".`);
    return { outcome: 'Negated (shield)', detail: 'A played protection card absorbed this event' };
  }

  // Defensive Maneuver covers "any card targeting you", so it falls in behind
  // the ordinary event shields rather than being wasted ahead of them. It is
  // the SAME charge the reaction window offers, so it cannot be spent twice.
  if ((player.attackShield || 0) > 0) {
    player.attackShield--;
    const src = player.attackShieldSource || 'Shield';
    addLog(s, `${player.name}'s ${src} negates "${name}".`);
    return { outcome: `Negated (${src})`, detail: 'Your one-time shield absorbed this event' };
  }

  // Talent negation riders — the printed "if this card is part of a project
  // selected for X, you can negate it" texts, implemented generically: an
  // ASSIGNED specialist absorbs their nemesis event for free.
  const riderName = TALENT_EVENT_RIDERS[name];
  if (riderName && hasAssignedTalent(player, riderName)) {
    addLog(s, `${riderName} handles it: "${name}" negated for ${player.name}.`);
    return { outcome: `Negated (${riderName})`, detail: `${riderName} is assigned and absorbs this event, as printed on the card` };
  }
  ...
  if (name === 'Production Outage') {
    ...
    recordIncomingHit(s, { target: playerIdx, source: -1, cardName: name, kind: 'freeze',
      detail: 'Production Outage: your projects make no progress this quarter.' });
  ...
  if (name === 'Uncertainty') {
    ...
    recordIncomingHit(s, { target: playerIdx, source: -1, cardName: name, kind: 'noAssign',
      detail: 'Uncertainty: you cannot put anyone onto a project this quarter.' });
...
export function resolveRushQ(card, s) {
  ...
  // Punished Senior prints "if this card is part of a project selected for the
  // Scope Creep event, you can negate it" — honoured as a per-player exemption.
  if (name === 'Scope Creep') {
    s.quarterFlags.noBonusProject = true;
    s.quarterFlags.bonusProjectExempt = [];
    for (let pi = 0; pi < s.players.length; pi++) {
      if (hasAssignedTalent(s.players[pi], SCOPE_CREEP_RIDER)) {
        s.quarterFlags.bonusProjectExempt.push(pi);
        addLog(s, `${SCOPE_CREEP_RIDER} shields ${s.players[pi].name} from Scope Creep.`);
      }
    }
  ...
  if (name === 'Freeze' || name === 'SOX Compliance') {
    ...
    // Recorded after the opt-outs above, so a player who unassigned a Manager
    // or paid a favour is not told about a freeze they successfully dodged.
    for (const pi of new Set(s.quarterFlags.frozenProjects)) {
      recordIncomingHit(s, { target: pi, source: -1, cardName: name, kind: 'freeze',
        detail: `${name}: your projects make no progress this quarter.` });
    }
    if (name === 'Freeze') {
      for (let pi = 0; pi < s.players.length; pi++) {
        recordIncomingHit(s, { target: pi, source: -1, cardName: name, kind: 'noProject',
          detail: 'Freeze: no new projects may be started this quarter.' });
      }
    }
  ...
  if (name === 'Appropriated Glory') {
    ...
    recordIncomingHit(s, {
      target: s.players.indexOf(bestTarget),
      source: s.players.indexOf(leader),
      cardName: name,
      kind: 'rep',
      amount: -stolen,
      detail: `${leader.name} took credit for your "${bestProject.name}".`,
    });
...
export function resolveLayoff(card, s) {
  ...
  if (target.reputation >= cost) {
    ...
    if (target.people.length) {
      const lostPerson = target.people[target.people.length - 1];
      target.people = target.people.filter(x => x.id !== lostPerson.id);
      for (const projId of Object.keys(target.assignments || {})) {
        target.assignments[projId] = (target.assignments[projId] || []).filter(id => id !== lostPerson.id);
      }
