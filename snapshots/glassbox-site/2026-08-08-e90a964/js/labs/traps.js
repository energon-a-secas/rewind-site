...
function pairCard(p) {
  const sides = p.sides.map((s) => `
    <div class="pair__side">
      ...
      <span class="pair__ref">${escHtml(s.ref)}</span>
    </div>`).join('');
  return `
    <article class="pair">
      <h3 class="pair__q">${escHtml(p.question)}</h3>
