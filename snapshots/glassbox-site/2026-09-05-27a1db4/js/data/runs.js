...
export const RUNS = {
  writing: {
    ...
    steps: [
      ...
      { chat: 'Read draft.md \u2713 (1,240 words)', raw: '\u2192 tool_result: draft.md \u00b7 1,240 words appended to context', turn: 'result', done: ['read'], active: 'coord', tokens: 3600, note: 'Tool results pile up in the context window. A 40-field result where 5 matter is wasted budget, so trim it with a PostToolUse hook.' },
      ...
      { chat: 'reviewer:grammar \u2713 4 line edits', raw: '\u2192 tool_result: reviewer:grammar \u2192 {findings:[\u2026], severity ranked} (JSON)', turn: 'result', done: ['grammar'], active: ['clarity', 'structure'], tokens: 5100, note: 'Subagents should return structured summaries, not raw dumps, which keeps the coordinator context lean.' },
      ...
      { chat: 'Coordinator: Merging 9 findings, dropping 2 duplicates, ranking by severity.', raw: '\u2190 (reasoning) aggregate + dedupe + rank across subagent outputs', turn: 'response', active: 'coord', tokens: 7200, note: 'All communication flows through the coordinator. Subagents never talk to each other, and that is what makes the run observable and debuggable.' },
  ...
  support: {
    ...
    steps: [
      ...
      { chat: 'Done. Your $89.99 refund lands in 3\u20135 days.', raw: '\u2190 stop_reason: "end_turn"', turn: 'response', done: ['agent'], tokens: 4100, note: 'Had the amount been $900, the hook would have blocked process_refund and forced escalate_to_human with a structured handoff, and no prompt could override it.' },
    ],
  },
