...
// `beyond: true` and render a badge. The exception
    ...
    verbs: [
      {
        ...
        trap: 'Ungrounded output invites fabrication, and attribution can’t be rebuilt afterwards, so reconstruction is the distractor shape.',
      },
      {
        term: 'trim',
      ...
      },
      {
        ...
        trap: 'It is a summary, so exact numbers and dates are the first casualties, so pin them somewhere verbatim first.',
      ...
      },
      {
        ...
        trap: 'The guide puts the internals out of scope, so on a guide-shaped stem an option hinging on cache mechanics is testing whether you’ll overthink. Third-party banks disagree and ask for the numbers directly, so learn the five of them anyway: a 1,024-token minimum (2,048 on Haiku), four <code>cache_control</code> breakpoints at most, a write at 1.25× base input, a read at 0.1×. Cheap to hold, and never the answer to a guide-shaped stem.',
      },
    ],
  },
];
...
export const DISTINCTIONS = [
  ...
  { a: 'consolidate', b: 'merge', diff: 'Remove redundancy, so fewer survive, vs preserve every part, nothing lost.' },
  ...
  { a: 'Resources', b: 'Tools', diff: 'MCP’s read-only context “maps” (schemas, catalogs, so you skip exploratory calls) vs actions the model invokes.' },
