...
| `search.js` | Hero search bar with floating category pills (physics simulation on canvas), card filtering, and constellation drawing connecting matched cards |
...
| `terminal.js` | Hidden terminal (keyboard shortcut) with Convex auth for admin commands |
...
1. Add card HTML in `index.html` with a unique `data-card-id`, appropriate `--card-glow`/`--card-accent`, and an SVG icon in `assets/icons/`.
2. Add the card ID to the relevant category in `CATEGORIES` array in `search.js`.
