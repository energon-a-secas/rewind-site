(function () {
  ...
  const CATEGORIES = [
    ...
    { label: 'Board Games',   color: '#e4483c', ids: ['rushq','cardforge','pieza','boardwright','enjeu'], keywords: 'enjeu boss rush solo one player print and play life cards bet guard bricks minifigure simulator board games tabletop cards card game deck print and play dice rush q corporate strategy quarters hire build ai opponents cardforge card designer editor template schema json export pieza dnd bilingual boss hunt improvised guardrails d20 plausibility boardwright board game design blueprint zones rules phases actions win conditions spec brief agent handoff uno colourful kids family playtest prototype' },
    { label: 'Fun',           color: '#f472b6', ids: ['decisionwheel','memes','clientsays','emojis','teamplay','gamebin','minimap','youtube','pixeldoll','vitrina','portent','mug'], keywords: 'mug mugs character mug 3d mug sculpted relief tiki teapot stein drinkware ceramic collectible abystyle paladone funko bigmouth silver buffalo shelf want list collectors vitrina bookshelf shelf books book collection library spines spine covers cover art paperbacks science fiction sci-fi sf nova ediciones b bolsillo zeta editions catalogue catalog isbn series saga missing volumes tercera fundacion spanish bookcase display case pixeldoll pixel character avatar builder sprite dress up wardrobe unlockables cookie floorplan minimap arpg parody dungeon boss grind level portent magic 8 ball eight ball fortune telling oracle prediction answers shake dice 3d toy embed iframe fun randomizer community upload wheel spin memes timezone translator jargon client says decoded emoji archive search gamebin game bin steam lists curate profile youtube video overflow pieza dice cards tabletop print play boss hunt dnd bilingual card game teamplay team building retros activities icebreakers games strategy' },
    { label: 'Social',        color: '#38bdf8', ids: ['dispatch','vibecheck','hiringpack','charactersheet','parla','playbook','tubestack','aficion','bouquin','sash','enamel'], keywords: 'sash enamel badges badge wallet certificates certificate credentials credly profile claim redeem recognition kudos meme badges open badges parody issuer studio editor designer shapes patterns fonts images export embed issue claim links expiring versions templates bouquin books book reddit suggestmeabook suggest me a book recommendations reading covers genres trending novels authors open library dispatch antenne news fleet feed changelog release notes launches updates stories headlines rss desk approval whats new social interviews scoring vibe check behavioral personality export character sheet know parla slang latin american regional language playbook career advice tech job hunting bilingual english spanish tubestack youtube channels discovery engineers match community hiring pack resume bullets follow-up aficion hobbies hobby atlas constellation map crafts skills passions pastimes gunpla model kits tabletop warhammer film horror ham radio coffee roasting cosplay bricks lego astronomy drones trains retro gaming shared gear one step away curated builds compare maps share link' },
    ...
    { label: 'Growth',        color: '#fcd34d', ids: ['gamme','mettle','primer','proctor','runcible','rappel','quiz'], keywords: 'quiz games rounds beats mora sound picker pairs order song kana hiragana katakana loanwords embed feedback why runcible book primer japanese kana hiragana katakana songs lyrics loanwords grammar kanji tutor learn language diamond age rappel flashcards spaced repetition srs fsrs anki decks review ledger embed memory cards gamme gambit game patterns minesweeper chess 2048 tollworks monopoly variant strategy drills solver deduction probability odds no-guess ruleset first click chording tactics fork pin skewer back rank corner anchor monotonicity expected value roi landing probability learn beat games quickly growth mettle reasoning maturity self assessment introspection virtues courage wisdom tolerance eloquence imagination rank probe character moral compass private primer machine learning ml llm ai data science insights training embeddings rag pytorch scikit-learn ollama tutorial learn beginner hands-on pet projects fortune 500 proctor exam test quiz simulator json yaml questions answers study certification' },
  ...
  function drawSonar(now) {
    /* Clamped because a rAF timestamp is stamped at frame start: the first
       frame after startSonar() can arrive with `now` a few ms older than
       sonarStart, and a negative age drove ctx.ellipse's radius negative,
       which throws. Age zero is the honest floor: the pulse has not left the
       centre yet. The reveal math in drawRoutes already clamps the same way. */
    var t = Math.max(0, (now - sonarStart) / SONAR_MS);
