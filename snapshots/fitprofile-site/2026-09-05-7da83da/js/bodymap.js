// ── Body map ─────────────────────────────────────────────────
// One SVG, two layers. The anatomical art is decoration and never receives
// pointer events; a separate region layer above it owns every hit test, so a
// hover always matches the shape the user sees.
import { BODY_ZONES, MAPPED_ZONES } from './data.js';
...
// The leg artwork is authored once and drawn twice, clipped above and below the
// ankle line, so "waist & legs" and "feet" can carry different tints.
const LEGS = {
  at: [57, 205],
...
// Hit regions, in the same user-unit space as the art. Every region clears the
// 44px touch-target guideline at the map's rendered size.
const HIT_REGIONS = {
  head: ['<rect x="68" y="-10" width="70" height="104" rx="30"/>'],
  torso: ['<path d="M58,62 H150 a6,6 0 0 1 5,7 L152,100 a6,6 0 0 1-4,6 L144,124 V236 a6,6 0 0 1-6,6 H68 a6,6 0 0 1-6-6 V124 L58,106 a6,6 0 0 1-4,-6 L51,69 a6,6 0 0 1 5,-7 Z"/>'],
  arm: [
    '<rect x="16" y="104" width="40" height="126" rx="18"/>',
    '<rect x="151" y="104" width="40" height="126" rx="18"/>',
  ],
  hands: [
    '<rect x="-6" y="230" width="48" height="54" rx="18"/>',
    '<rect x="165" y="230" width="48" height="54" rx="18"/>',
  ],
  'waist-legs': ['<rect x="56" y="242" width="96" height="180" rx="22"/>'],
  feet: ['<rect x="56" y="424" width="96" height="74" rx="22"/>'],
};

/** Render the interactive body map. */
export function renderBodyMap(activeZone) {
  const artLayer = Object.entries(ART)
    .map(([name, part]) => `<path class="art-part art-${name} ${zoneClassFor(name)}" transform="translate(${part.at[0]},${part.at[1]})" d="${part.d}"/>`)
    .join('');

  const hitLayer = MAPPED_ZONES.map(id => {
    const zone = BODY_ZONES.find(z => z.id === id);
    const isActive = activeZone === id;
    return `
      <g class="hit-zone${isActive ? ' is-active' : ''}" data-zone="${id}"
  ...
  return `
    <svg class="body-map" viewBox="-8 -14 223 516" role="group"
      ...
      <defs>
        ...
        <clipPath id="fp-clip-legs" clipPathUnits="userSpaceOnUse">
          <path d="M56,203 H152 V422.8 L120.7,423.1 L108.5,405.6 L99.1,405.6 L87,423.1 L56,422.8 Z"/>
        </clipPath>
        <clipPath id="fp-clip-feet" clipPathUnits="userSpaceOnUse">
          <path d="M56,422.8 L87,423.1 L99.1,405.6 L108.5,405.6 L120.7,423.1 L152,422.8 V494 H56 Z"/>
        </clipPath>
      </defs>

      <g class="body-art" aria-hidden="true">
        ${artLayer}
        ${clippedLegs('waist-legs', 'fp-clip-legs')}
        ${clippedLegs('feet', 'fp-clip-feet')}
      </g>

      <g class="body-hit">${hitLayer}</g>
    </svg>
  `;
}

/**
 * One half of the shared legs artwork, tinted for its own zone.
 * The styled element wraps the clip rather than carrying it: a `drop-shadow`
 * applied inside a clip is cut off at the clip edge and reads as a hard box.
 */
function clippedLegs(zoneId, clipId) {
  return `
    <g class="art-part zone-${zoneId}">
      <g clip-path="url(#${clipId})">
        <use href="#fp-legs" transform="translate(${LEGS.at[0]},${LEGS.at[1]})"/>
      </g>
    </g>`;
}

/** Map an art part name to the zone whose tint it should follow. */
function zoneClassFor(partName) {
  if (partName === 'head') return 'zone-head';
  if (partName === 'arm') return 'zone-arm';
  if (partName === 'hands') return 'zone-hands';
  return 'zone-torso';
}
